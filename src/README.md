...
- **Gestão de Usuários e Unidades**: As páginas `/users` e `/units` são o centro de controle administrativo, acessíveis **apenas para usuários com a função `Admin`**. Nestas seções, o administrador pode criar, editar e excluir usuários, definir suas funções e a quais unidades eles pertencem. Da mesma forma, pode cadastrar novas clínicas (unidades) e gerenciar suas informações básicas.
- **Logs**: A página `/logs` exibe um feed de auditoria de todas as ações importantes, gravadas na coleção `logs` pela função `createLog`. Apenas **Admins** podem visualizar o histórico completo de atividades do sistema.
