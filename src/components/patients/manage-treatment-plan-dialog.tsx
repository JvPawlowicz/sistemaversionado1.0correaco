...
import { Loader2, Plus, Trash2, GripVertical, CircleAlert } from 'lucide-react';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select';
import { useToast } from '@/hooks/use-toast';
import { updatePatientTreatmentPlanAction } from '@/lib/actions';
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl h-[90vh] flex flex-col">
        ...
        <form ref={formRef} onSubmit={handleFormSubmit} className="flex-1 flex flex-col min-h-0">
          {error && <div className="p-3 mb-4 bg-destructive/10 text-destructive text-sm rounded-md flex items-center gap-2"><CircleAlert className="h-4 w-4" />{error}</div>}
              ...
              {plan.goals.map((goal, goalIndex) => (
                <div key={goal.id} className="p-4 border rounded-lg space-y-4 bg-card">
                  <div className="flex items-center gap-2">
                    ...
                    <Button variant="ghost" size="icon" className="text-destructive ml-auto" onClick={() => handleRemoveGoal(goalIndex)}>
                  ...
                  <div className="pl-4 border-l-2 border-primary/50 space-y-3">
                    ...
                    {goal.objectives.map((objective, objectiveIndex) => (
                      <div key={objective.id} className="p-3 border rounded-md bg-background space-y-3">
                        <div className="flex items-center gap-2">
                          ...
                          />
                           <Button variant="ghost" size="icon" className="text-destructive" onClick={() => handleRemoveObjective(goalIndex, objectiveIndex)}>
                            <Trash2 className="h-4 w-4" />
                          </Button>
                        </div>
                    ...
                    <Button variant="outline" size="sm" onClick={() => handleAddObjective(goalIndex)}>
                      <Plus className="mr-2 h-4 w-4" /> Adicionar Objetivo
                    </Button>
                  </div>
                </div>
              ))}
              <Button variant="secondary" className="w-full" onClick={handleAddGoal}>
